...
@onready var victory_scene: PackedScene = preload("res://Scenes/PantallaDeGanaste.tscn")
...
func instantiate_npc_hands():
	for i in range(num_npcs):
		...
		if i in npc_indicators:
			npc_indicators[i].texture = preload("res://Sprites/pixil-frame-0 (1).png")
...
func handle_npc_turn():
	...
	update_npc_indicator(current_npc_index, true)
...
func finish_npc_turn():
	update_npc_indicator(current_npc_index, false)
...
func _on_player_card_discarded(card: MainCards):
	...
	# Victoria instantánea en cuanto se queda sin cartas
	if hand.is_empty():
		game_over = true
		player_won = true
		return
...
# ------------------------------------------------------------------
func update_npc_indicator(index: int, active: bool):
	if index in npc_indicators:
		var texture_path = "res://Sprites/pixil-frame-0.png" if active else "res://Sprites/pixil-frame-0 (1).png"
		npc_indicators[index].texture = load(texture_path)
